get_catalog_censo_superior <-
  function( data_name = "censo_superior" , output_dir , ... ){

    inep_portal <- "http://portal.inep.gov.br/microdados"

    w <- rvest::html_attr( rvest::html_nodes( xml2::read_html( inep_portal ) , "a" ) , "href" )

    these_links <- grep( "_superior(.*)zip$" , w , value = TRUE , ignore.case = TRUE )

    censo_superior_years <- substr( gsub( "[^0-9]" , "" , these_links ) , 1 , 4 )

    catalog <-
      data.frame(
        year = censo_superior_years ,
        dbfolder = paste0( output_dir , "/MonetDB" ) ,
        output_folder = paste0( output_dir , "/" , censo_superior_years ) ,
        full_url = these_links ,
        stringsAsFactors = FALSE
      )

    catalog <- catalog[ with( catalog , order( year ) ) , ]

    catalog[ catalog$year >= 2008 , ]

  }


lodown_censo_superior <-
  function( data_name = "censo_superior" , catalog , path_to_7z = if( .Platform$OS.type != 'windows' ) '7za' else normalizePath( "C:/Program Files/7-zip/7z.exe" ) , ... ){
    ...
    for ( i in seq_len( nrow( catalog ) ) ){
      ...
      unzipped_files <- lodown:::unzip_warn_fail( tf , exdir = catalog[ i , "output_folder" ] )

      for ( these_zips in grep( "\\.zip$" , unzipped_files , value = TRUE , ignore.case = TRUE ) ) unzipped_files <- c( unzipped_files , unzip_warn_fail( these_zips , exdir = lodown:::np_dirname( these_zips ) ) )

      if( catalog[ i , 'year' ] <= 2006 ){

        sas_files <- grep( "\\.sas$", unzipped_files, value = TRUE , ignore.case = TRUE )

        sas_scaledowns <- gsub( "_SAS|^_|_$" , "" , gsub( "\\.sas|\\.SAS" , "" , gsub( paste0( "INPUT|" , catalog[ i , 'year' ] ) , "" , basename( sas_files ) ) ) )
        sas_scaledowns <- ifelse( grepl( "ESC" , sas_scaledowns ) & !grepl( "INDICESC" , sas_scaledowns ) , gsub( "ESC" , "" , sas_scaledowns ) , sas_scaledowns )

        datafile_matches <- lapply( sas_scaledowns , function( z ) unzipped_files[ grepl( z , basename( unzipped_files ) , ignore.case = TRUE ) & grepl( "dados" , lodown:::np_dirname( unzipped_files ) , ignore.case = TRUE ) ] )
        datafile_matches <- lapply( datafile_matches , function( z ) z[ grepl( "\\.txt$" , z , ignore.case = TRUE ) ] )
        ...
        for ( this_table_type in c( "aluno" , "docente" , "curso" , "ies" , "oferta" ) ) {
              ...
              suppressMessages(
                tryCatch(
                  DBI::dbWriteTable(
                    ...
                    cleaned_data_file <- remove_nonutf8_censo_superior( this_data_file ) ;
                    DBI::dbWriteTable(
                      db,
                      ifelse( length( tabelas ) > 1 , tolower( gsub( "\\..*" , "" , basename( this_data_file ) ) ) , paste0( this_table_type , "_" , catalog[ i , "year" ] ) ),
                      cleaned_data_file ,
